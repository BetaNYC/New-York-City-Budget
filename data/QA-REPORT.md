...
**Verdict:** PASS — 272 files, 0 hard failure(s), 443 soft advisory(ies).
...
|---|---|---|---|---|
| `combined/all_years_awards.csv` | 33638 | 100% | 0 | duplicate: 143 duplicate row instance(s); e.g. x2: ['FY17', 'HOUSING', 'Housing', 'Community Housing Preservation Strategies']...; column_bleed: 106 suspected surname-in-organization residual(s); e.g. line 489: 'Hudson Guild'; org_prose: 438 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 5854: 'Funding to support neighborhood activities; including a chil'; org_merged: 276 award row(s) with an EIN or `$` inside `organization` — row boundary lost, so `amount` may belong to a different org than `organization` names; e.g. line 672: "Center for Community Alternatives' Crossroads Program 16-139" |
...
| `fy18/schedule_c/fy18_appendix_a_aging.csv` | 422 | 100% | 0 | org_prose: 270 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 3: '.00 Funds will go to G.R.A.N.N.Y program, which provides sen'; org_merged: 27 award row(s) with an EIN or `$` inside `organization` — row boundary lost, so `amount` may belong to a different org than `organization` names; e.g. line 11: ".00 Funds will support AAFE's senior care center in District" |
...
| `fy21/schedule_c/fy21_appendix_a_aging.csv` | 514 | 100% | 0 | org_prose: 4 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 122: 'Funding to support senior programming and activities includi' |
| `fy21/schedule_c/fy21_appendix_b_local.csv` | 2902 | 100% | 0 | duplicate: 1 duplicate row instance(s); e.g. x2: ['Richards', 'Queens Borough Public Library', 'Arverne Library', '136400434']...; column_bleed: 46 suspected surname-in-organization residual(s); e.g. line 24: 'Adams Street Foundation, Inc.'; org_prose: 19 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 258: 'To support BRIC’s free and accessible youth and family progr' |
| `fy21/schedule_c/fy21_appendix_c_youth.csv` | 894 | 100% | 0 | column_bleed: 2 suspected surname-in-organization residual(s); e.g. line 416: 'Hudson Guild'; org_prose: 6 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 101: 'To provide funding to support the teen interns participating' |
| `fy21/schedule_c/fy21_schedule_c_awards.csv` | 1810 | 100% | 0 | duplicate: 4 duplicate row instance(s); e.g. x2: ['Boroughwide Needs', '', 'member_item', 'Brooklyn']...; column_bleed: 3 suspected surname-in-organization residual(s); e.g. line 1031: 'Hudson Guild'; org_prose: 36 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 336: 'Funding to support neighborhood activities; including a chil' |
...
| `fy22/schedule_c/fy22_appendix_a_aging.csv` | 510 | 100% | 0 | org_prose: 32 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 32: 'Funding to support cultural and educational lectures and exc' |
| `fy22/schedule_c/fy22_appendix_b_local.csv` | 2790 | 100% | 0 | duplicate: 1 duplicate row instance(s); e.g. x2: ['Eugene', 'Prospect Lefferts Gardens Neighborhood Association, Inc.', '', '237064386']...; column_bleed: 55 suspected surname-in-organization residual(s); e.g. line 12: 'Ayala 2020 Vision for Schools, Inc.'; org_prose: 111 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 44: 'Funding to support Afro Latin Jazz Academy of Music Residenc' |
| `fy22/schedule_c/fy22_appendix_c_youth.csv` | 882 | 100% | 0 | column_bleed: 3 suspected surname-in-organization residual(s); e.g. line 411: 'Hudson Guild'; org_prose: 42 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 34: 'Funds will support supplies, equipment, and other needs to p' |
| `fy22/schedule_c/fy22_schedule_c_awards.csv` | 1492 | 100% | 0 | duplicate: 1 duplicate row instance(s); e.g. x2: ['Speaker’s Initiative to Address Citywide Needs', 'Speaker’s Initiative to Address Citywide Needs', 'initiative_provider', '']...; column_bleed: 5 suspected surname-in-organization residual(s); e.g. line 409: 'Dinowitz, Riley New York Botanical Garden'; org_prose: 63 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 372: "Funds will be utilized in support of the Department's emerge" |
...
| `fy23/schedule_c/fy23_appendix_a_aging.csv` | 489 | 100% | 0 | org_prose: 30 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 42: 'Funding to support Braata singing programming in senior cent' |
| `fy23/schedule_c/fy23_appendix_b_local.csv` | 2726 | 100% | 0 | duplicate: 1 duplicate row instance(s); e.g. x2: ['Brooks-Powers', 'Queens Borough Public Library', 'Laurelton Library', '136400434']...; column_bleed: 118 suspected surname-in-organization residual(s); e.g. line 29: 'Krishnan, Won Adhikaar for Human Rights and Social Justice'; org_prose: 139 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 14: 'Funding to support costs associated with New Play Developmen' |
| `fy23/schedule_c/fy23_appendix_c_youth.csv` | 841 | 100% | 0 | column_bleed: 2 suspected surname-in-organization residual(s); e.g. line 410: 'Hudson Guild'; org_prose: 51 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 21: 'Funds will support the operations of the Jill Chaifetz Educa' |
| `fy23/schedule_c/fy23_schedule_c_awards.csv` | 1848 | 100% | 0 | duplicate: 6 duplicate row instance(s); e.g. x3: ['BOROUGHWIDE NEEDS', '', 'member_item', 'Manhattan']...; column_bleed: 7 suspected surname-in-organization residual(s); e.g. line 1394: 'Marte, Schulman Association of Community Employment Programs'; org_prose: 84 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 374: 'Funds will be used to support stewardship for children & adu' |
...
| `fy24/schedule_c/fy24_appendix_a_aging.csv` | 477 | 100% | 0 | org_prose: 1 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 35: 'will support a NORC program at the Philip Howard Houses. It ' |
| `fy24/schedule_c/fy24_appendix_b_local.csv` | 2616 | 100% | 0 | duplicate: 1 duplicate row instance(s); e.g. x2: ['Restler', 'Department of Education', 'Urban Assembly Institute of Math and Science for Young Women (K527)', '136400434']...; column_bleed: 89 suspected surname-in-organization residual(s); e.g. line 101: 'Brewer Aperture Foundation, Inc.'; org_prose: 2 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 568: 'at CCNY. It will support public events focuses on LGBTQ+ the' |
...
| `fy24/schedule_c/fy24_schedule_c_awards.csv` | 5368 | 100% | 0 | duplicate: 28 duplicate row instance(s); e.g. x2: ['CULTURAL ORGANIZATIONS', 'Cultural After-School Adventure (CASA)', 'member_item', 'Williams']...; column_bleed: 22 suspected surname-in-organization residual(s); e.g. line 394: 'Hudson Guild'; org_prose: 62 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 4732: 'Funding to support staff and to provide health education, pe'; org_merged: 1 award row(s) with an EIN or `$` inside `organization` — row boundary lost, so `amount` may belong to a different org than `organization` names; e.g. line 4850: 'The funds requested will subsidize the delivery of farm shar' |
...
| `fy25/schedule_c/fy25_appendix_b_local.csv` | 2616 | 100% | 0 | column_bleed: 85 suspected surname-in-organization residual(s); e.g. line 163: 'Holden Association of Community Employment Programs for the '; org_prose: 1 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 1621: 'CDC, Inc. (NHS Brooklyn) to provide direct services to homeo' |
...
| `fy25/schedule_c/fy25_schedule_c_awards.csv` | 5646 | 100% | 0 | duplicate: 18 duplicate row instance(s); e.g. x3: ['CULTURAL ORGANIZATION', 'Cultural After-School Adventure (CASA)', 'member_item', 'Williams']...; column_bleed: 14 suspected surname-in-program residual(s); e.g. line 212: 'Louis Pink Houses TA Programming'; org_prose: 58 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 3224: 'The funding will pay for e-waste events in Council District '; org_merged: 1 award row(s) with an EIN or `$` inside `organization` — row boundary lost, so `amount` may belong to a different org than `organization` names; e.g. line 5074: 'The funds requested will subsidize the delivery of farm shar' |
...
| `fy26/schedule_c/fy26_schedule_c_awards.csv` | 5838 | 100% | 0 | duplicate: 15 duplicate row instance(s); e.g. x2: ['CULTURAL ORGANIZATIONS', 'Cultural After-School Adventure (CASA)', 'member_item', 'Williams']...; column_bleed: 16 suspected surname-in-program residual(s); e.g. line 200: 'Louis Armstrong Houses TA Association'; org_prose: 62 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 5023: 'Funding to support staff and to provide health education, pe'; org_merged: 1 award row(s) with an EIN or `$` inside `organization` — row boundary lost, so `amount` may belong to a different org than `organization` names; e.g. line 5204: 'The funds requested will subsidize the delivery of farm shar' |
...
| `fy27/schedule_c/fy27_schedule_c_awards.csv` | 6118 | 100% | 0 | duplicate: 33 duplicate row instance(s); e.g. x2: ['Cultural Organizations', 'Cultural After-School Adventure (CASA)', 'member_item', 'Abreu']...; column_bleed: 16 suspected surname-in-program residual(s); e.g. line 203: 'Louis Armstrong Houses TA Association'; org_prose: 73 award row(s) whose `organization` holds purpose prose instead of a grantee name — `ein` and `amount` are intact, the display name is lost; e.g. line 306: 'Funding will be used for projects that strengthen the Bay Ri' |

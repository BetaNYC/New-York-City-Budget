...
## 13. Schedule C — repeated page-break header dropped whole award rows — RESOLVED (full corpus FY2021–FY2025 + FY2027; FY2026 held back, see below)
...
FY2016–FY2020 use the older narrow format and have no repeated wide "with-purpose" header — **0 change**. Category reconciliation is untouched in every year (the fix is in `parse_awards`, not `parse_initiatives`); each year's `*_schedule_c_initiatives.csv` and the appendix CSVs are byte-identical to before. All recovered rows carry real organization names (100% valid EIN coverage per `validate_data.py`).

**FY2026 held back (pre-existing, unrelated blocker).** FY2026 could not be regenerated in this pass. Its committed reconciliation reports 25 ToC categories, but the current parser + `pypdf 6.14.2` cannot detect FY2026's Table of Contents — the ToC lines are present on source PDF p.3 but that page carries no standalone "Contents"/"Table of Contents" heading, so `derive_categories()` gates them out and returns 0 categories. Regenerating FY2026 would therefore wipe every award's category label (5,838 empty-category rows) and collapse reconciliation to 0/0. This is a **pre-existing `derive_categories` fragility, independent of this PR's changes** (both the pre-fix and Issue-1 parsers reproduce the 0-category result today). FY2026 is left as committed pending a separate fix to `derive_categories` (detect a ToC page by dotted-entry density when no heading line is present). Consequence: FY2026 Schedule C awards still carry the header-bleed drops (~37 rows) and the "Delegation Fund" mislabel (§14) until that fix lands.
...
## 14. Schedule C — "Delegation Fund for the City of New York" mislabel + org-name truncations — RESOLVED (full corpus except FY2026)
...
**Outcome.** The string **"Delegation Fund" no longer appears in any regenerated Schedule C CSV**, and no organization leads with the bled "Delegation" token. Per-year org relabelings (all verified as correct, not new mislabels): FY2021–FY2025 + FY2027 each drop ~120–165 "Delegation X" → "X"; FY2016 corrected 5 FCNY rows that the old backfill had mislabeled "Floating Hospital"/"Man Up"; FY2017/FY2018 cleaned mangled FCNY rows; FY2020 corrected "District 4" → "Find Aid for the Aged, Inc."; FY2019 was org-neutral. Category reconciliation is unchanged in every year. Regression tests: `test_fund_org_name_vs_purpose_prose` (both directions — org preserved, purpose still detected) and the FCNY/ICARE/NYCETC/no-leading-"Delegation" assertions in `test_fy27_west_side_work_coalition_survives`.

**Not covered here:** (a) **FY2026** — held back by the ToC blocker in §13, so its "Delegation Fund" rows remain until that is fixed. (b) The **borough delegation** still lands in the `member` field as "<Borough> Delegation" (e.g. "Bronx Delegation") — correct sponsor attribution, not a defect. (c) A *parallel* "Delegation"/purpose bleed exists in the **Transparency Resolutions** output (`parse_transparency_reso.py`, a different parser — e.g. "Delegation Funding will support…"); it is a separate code path, out of scope for this Schedule C fix, and flagged for a follow-up. (d) Two FY2027 rows still show purpose text as the organization (EINs 92-1532576, 93-3219755) — the pre-existing MI-mode multi-designation mangle (§12 residual class), unchanged by this PR.

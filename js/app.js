(function(){
    ...
    //
    // BetaNYC Schedule C adaptation. The three hierarchy column names in the CSV
    // (Fund / Control Officer / Department) are hard-coded throughout this file, so
    // they are KEPT and repurposed semantically (see viz/PLAN.md):
    //   Fund            = Schedule C category      (axis 1, "Where's it going?")
    //   Control Officer = administering agency      (axis 2, "Who administers it?")
    //   Department      = initiative                (the shared leaf)
    // The per-year column key is `apropTitle + ' ' + year` / `expendTitle + ' ' + year`,
    // so these two labels MUST equal the column prefixes emitted by schedulec_cleanup.py.
    startYear   = 2015;  // first fiscal year in data/{year}/schedule_c/
    endYear     = 2027;  // last fiscal year in the repo
    activeYear  = 2027;  // default year (FY2027 reconciles to $655,764,999)
    debugMode   = false; // change to true for debugging message in the javascript console
    municipalityName = 'NYC Council Discretionary Funding'; // shown in breadcrumb + title
    apropTitle  = 'Adopted';  // line 1: adopted Schedule C total (authoritative, reconciled)
    expendTitle = 'Itemized'; // line 2: dollars designated to a named recipient (see PLAN.md)

    // CSV data source. RELATIVE path (not '/data/...') on purpose: this viz lives in a
    // /viz subfolder of a repo that ALSO has a top-level /data tree; an absolute '/data/'
    // would collide with the repo's source CSVs. Serve the site from inside viz/.
    dataSource  = 'data/schedule_c.csv';
